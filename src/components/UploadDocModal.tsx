...
export default function UploadModal({
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!ALLOWED_MIME_TYPES.some(({ type }) => type === file.type)) {
      toast.error(
        "Unsupported file type. Supported formats: TXT, DOCX, JSON, HTML, JPEG, PNG, PDF."
      );
      setLoading(false);
      return;
    }
  ...
  return (
    <>
      {!isSecondModalOpen ? (
        <div className="bg-white rounded-lg shadow-lg p-6 w-[90%] md:w-[48rem]">
          ...
          {loading ? (
            ...
            <div className="flex flex-col md:flex-row gap-4 md:gap-10">
              ...
              <label className="w-full md:w-2/3 h-44 border-2 border-dashed border-gray-300 rounded-lg flex flex-col items-center justify-center gap-3 p-4 hover:shadow-lg cursor-pointer">
                <p className="text-gray-500 text-center text-sm md:text-base">
                  Drag and drop your file (txt, json, docx, html)
                </p>
              ...
              <div
                className="w-full md:w-1/3 h-44 border rounded-lg flex flex-col items-center justify-center gap-3 p-4 hover:shadow-lg cursor-pointer"
        ...
        <div className="bg-white rounded-lg shadow-lg p-6 w-[85%] md:w-[28rem]">
          <div className="flex justify-between items-center">
